...
<input type="hidden" id="status2" value="<%= request.getAttribute("status2") %>">
	...
	<script type="text/javascript">
		var status1 = document.getElementById("errorMessage").value;
		if(status1 == "error"){
			swal("Sorry", "All fields are mandatory","error");
		}
	</script>
	<script type="text/javascript">
		var status = document.getElementById("status1").value;
		if(status == "failed"){
			swal("status1", "email already exists","error");
		}
	</script>
	<script type="text/javascript">
		var status = document.getElementById("status2").value;
		if(status == "error"){
			swal("status2", "Password does not match","error");
		}
	</script>
